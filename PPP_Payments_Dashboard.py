    ...
    with tab3:
        ...
        with col2:
            if trans_state != 'Select' and trans_state_year != 'Select' and trans_state_quarter != 'Select':
                ...
                total_count_formatted = locale.format("%d", total_count, grouping=True)
                average_count_formatted = locale.format("%d", average_count, grouping=True)  # Display average with 0 decimal places
                total_amount_formatted = "₹ " + locale.format("%d", total_amount, grouping=True)  # Add Rupee symbol and commas
                average_amount_formatted = "₹ " + locale.format("%d", average_amount, grouping=True)
